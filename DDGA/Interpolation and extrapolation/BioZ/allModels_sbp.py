...
for subject in subjects:
    ...
    for testBucket in buckets.keys():
        ...
        sbp_train = sbp_data[np.asarray(train_indices)]

        x_test = X_data[np.asarray(test_indices)]     
        x_dtw_test = X_dtw_data[np.asarray(test_indices)]
        sbp_test = sbp_data[np.asarray(test_indices)]
        ...
        save_directory = "Results/bilstm_attn/personalized/sbp/" + exp + "/"    
        ...
        model.fit(X_train, sbp_train, validation_split=0.1, shuffle=True, epochs=50, batch_size=16, callbacks=callbacks_list)
        ...
        np.save(save_directory + subject + '/' + testBucket + '/BP_test',sbp_test)
        np.save(save_directory + subject + '/' + testBucket + '/preds',preds)

        K.clear_session()

        save_directory = "Results/GA_bilstm_attn/personalized/sbp/" + exp + "/"
        if not os.path.exists(save_directory):
            os.makedirs(save_directory)
        ...
        model.fit(X_train, [ch1_X_dtw_train,ch2_X_dtw_train,sbp_train], validation_split=0.1, shuffle=True, epochs=50, batch_size=16, callbacks=callbacks_list)
        ...
        np.save(save_directory + subject + '/' + testBucket + '/preds_2',preds[2])
        ...
        save_directory = "Results/cnn_bilstm_attn/personalized/sbp/" + exp + "/"    
        if not os.path.exists(save_directory):
            os.makedirs(save_directory)
        ...
        save_directory = "Results/GA_cnn_bilstm_attn/personalized/sbp/" + exp + "/"
        if not os.path.exists(save_directory):
            os.makedirs(save_directory)
        ...
        save_directory = "Results/transformer/personalized/sbp/" + exp + "/"    
        if not os.path.exists(save_directory):
            os.makedirs(save_directory)
        ...
        save_directory = "Results/GA_transformer/personalized/sbp/" + exp + "/"
        if not os.path.exists(save_directory):
            os.makedirs(save_directory)
